...
class CityDataset(Dataset):
    ...
    def __init__(self, 
        ...
        if images is not None and buildings_mask is not None and boundaries_mask is not None:
            ...
            raise ValueError("Either path or (data, labels and boundaries_mask) must be provided")
    ...
    def __getitem__(self, idx):
        item = {"data": self.X[idx][self.__image_bands], "labels": np.array([self.y[idx]])}
        if self.transform:
    ...
    def train_val_split(self, val_size=0.2, n_groups=100, random_state=42, show_summary=True):
        """
    ...
    def __print_summary(self, train_ds, val_ds, train_indices, val_indices):
        ...
        print("Shape of Train data (data, label)",train_ds[0]["data"].shape, train_ds[0]["labels"].shape)
        print("Val:")
        print("Number of samples:", len(val_ds))
        print("Shape of Val data (data, label)",val_ds[0]["data"].shape, val_ds[0]["labels"].shape)
        # get the mean percentage of labels
        print("*"*50)
        print("Mean percentage of 1 labels in train:", np.mean([np.sum(d["labels"]) / (self.__patch_size ** 2) for d in train_ds]))
        print("Mean percentage of 1 labels in val:", np.mean([np.sum(d["labels"]) / (self.__patch_size ** 2) for d in val_ds]))
        print("Mean percentage of 1 labels in all data:", np.mean([np.sum(d["labels"]) / (self.__patch_size ** 2) for d in self]))
        print("*"*50)
        print(f"Std of percentage 1 labels in train: {np.std([np.sum(d['labels']) / (self.__patch_size ** 2) for d in train_ds])}")
        print(f"Std of percentage 1 labels in val: {np.std([np.sum(d['labels']) / (self.__patch_size ** 2) for d in val_ds])}")
        print(f"Std of percentage 1 labels in all data: {np.std([np.sum(d['labels']) / (self.__patch_size ** 2) for d in self])}")
        print("*"*50)
        print(f"Min percentage of 1 labels in train: {np.min([np.sum(d['labels']) / (self.__patch_size ** 2) for d in train_ds])}")
        print(f"Min percentage of 1 labels in val: {np.min([np.sum(d['labels']) / (self.__patch_size ** 2) for d in val_ds])}")
        print(f"Min percentage of 1 labels in all data: {np.min([np.sum(d['labels']) / (self.__patch_size ** 2) for d in self])}")
        print("*"*50)
        print(f"Max percentage of 1 labels in train: {np.max([np.sum(d['labels']) / (self.__patch_size ** 2) for d in train_ds])}")
        print(f"Max percentage of 1 labels in val: {np.max([np.sum(d['labels']) / (self.__patch_size ** 2) for d in val_ds])}")
        print(f"Max percentage of 1 labels in all data: {np.max([np.sum(d['labels']) / (self.__patch_size ** 2) for d in self])}")
        print("*"*50)
        ...
        df = pd.DataFrame({
            ...
            "Validation":pd.Series(validation_citiy_counts[1]/val_cities.shape[0], index=validation_citiy_counts[0], name='validation')})
    ...
    def __create_image_patches(self):
        ...
        X = []
        y = []
        for i,( d, l) in tqdm(enumerate(zip(self.data, self.labels)), desc="Creating Patches from Images"):
    ...
    def __create_image_patches_from_image(self, image, label_image, idx):
        ...
        for i in range(start_width, (width - self.__patch_size - start_width + 1), self.__stride):
            for j in range(start_height, (height - self.__patch_size - start_height + 1), self.__stride):
                

                #  check if within the osm boundaries
                if self.__boundaries_mask is not None:
                    boundaries_patch = self.__boundaries_mask[idx][i:i+self.__patch_size, j:j+self.__patch_size]
                    if any(boundaries_patch.flatten() < 1):
                        continue
                # check if the patch has enough labels
                label_patch = label_image[i:i+self.__patch_size, j:j+self.__patch_size]
    ...
    def __load_data(self, data_name, labels_name): 
        directories = [d for d in os.listdir(self.__path) if os.path.isdir(os.path.join(self.__path, d)) and not d.startswith(".")]
        
        if self.__cities:
            directories = [d for d in directories if d in self.__cities]

        # check if there is a file with name data_name in each directory
        assert all([data_name in os.listdir(os.path.join(self.__path, d)) for d in directories]), f"Data file {data_name} not found in all directories"
        assert all([labels_name in os.listdir(os.path.join(self.__path, d)) for d in directories]), f"Labels file {labels_name} not found in all directories"
        data_city_paths = [os.path.join(self.__path, d, data_name) for d in directories]
        labels_city_paths = [os.path.join(self.__path, d, labels_name) for d in directories]
